        ...
        <div>
            <v-row style="margin-top: 28px;">
                    ...
                    <v-col cols="12" md="6">
                        ...
                        <v-btn  class="icon"  @click.prevent="add_country()"  :loading="isloading" append-icon="mdi:arrow-right" variant="tonal" height="60px" width="100%" rounded
...
<script setup>
    import { addCountries } from "@/composables/requests/country"
    ...
    const minimum_fiat_funding = ref(null)
    const maximum_fiat_funding = ref(null)
    const minimum_fiat_to_crypto_swap = ref(null)
    const minimum_withdrawal = ref(null)
    const withdrawal_percentage_fee = ref(null)
    const supported_payment_gateways = ref([])
    ...
    const add_country = async()=>{
        ...
        try{
            const data = await addCountries(payload)
            if(data.success){
