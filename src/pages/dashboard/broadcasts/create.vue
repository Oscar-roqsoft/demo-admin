            ...
            <v-row>
                ...
                <v-col sm="12" md="10" style="margin-top: 16px;">
                    ...
                    <v-col cols="12" md="6">
                        ...
                        <v-btn  class="icon"  @click.prevent="add_broadcast()"  :loading="isloading" append-icon="mdi:arrow-right" variant="tonal" height="60px" width="100%" rounded
...
<script setup>
    import { addBroadcast } from "@/composables/requests/broadcast"
    ...
    const title = ref('')
    const broadcast_type = ref("")
    const message = ref("")
    const country_group = ref([])
    const  myfile = ref(null)
    const  preview = ref(null)
    const file_url = ref(null)
    const isPublishOn = ref('')
    ...
    const add_broadcast = async()=>{
        ...
        try{
            const data = await addBroadcast(payload)
            if(data.success){
