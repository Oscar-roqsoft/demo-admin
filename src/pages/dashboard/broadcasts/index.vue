    ...
    <div class="section-con" style="background:  #161D26; min-height: 100vh; padding: 33px 36px;;">
        ...
        <v-row>
            ...
            <v-col style="padding:0px 16px; align-items: center;" class="d-flex flex-wrap justify-space-between  inner-heading">
                <h1 class="title" style="color: #FFFFFF; font-size: 24px; font-weight: 700;"> All Broadcast</h1>
                <v-btn @click="navigateTo('/dashboard/broadcasts/create')" color="#0CAF60" style="height: 50px;min-width: 150px; border-radius: 20px; text-transform: capitalize;color:#FFFFFF">Create</v-btn>                          
            </v-col>
        ...
        <v-row  style="padding: 24px 0px; margin: 16px auto;" >
            <v-col cols="12" md="6" lg="4" v-for="i in pinia.state.broadcasts" :key="i.id">
                ...
                >
                    ...
                    <v-card-actions>
                    <v-btn @click="navigateTo(`/dashboard/broadcasts/${i.id}`)" variant="tonal" color="#0CAF60" style="text-transform: capitalize;">Edit</v-btn>
                    ...
                    <v-card  style="background: #12181F !important; color: #FFFFFF; border-radius: 15px;">
                        ...
                        <v-btn color="" :loading="isDeleting"  @click="delete_broadcast" style="background: red; font-size: medium; text-transform: capitalize; border-radius: 10px;">Confirm</v-btn>
...
<script setup>
    import { deleteBroadcast } from "@/composables/requests/broadcast"

// import {  } from "@/composables/requests/broadcast"
...
const delete_broadcast = async() => {
    isDeleting.value = true
    try{
        const data = await deleteBroadcast(filteredItem.value.id)
        if(data.success){
            push.success('Deleted successfully')  
            const filtered = pinia.state.broadcasts.filter(e => e.id !=  filteredItem.value.id )
            pinia.setBroadcasts(filtered)
        }else{
