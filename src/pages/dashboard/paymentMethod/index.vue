        ...
        <div style="padding: 33px 26px;" class="section-con">
                <v-row>
                    ...
                    <v-col style="padding:0px 24px; align-items: center; display: flex;" class="d-flex flex-wrap justify-space-between  inner-heading">
                         <h1 class="title" style="color: #FFFFFF; font-size: 22px; font-weight: 700;"> Payment Methods</h1>
                          <v-btn @click="navigateTo('/dashboard/paymentMethod/create')"  color="#0CAF60" style="height: 50px; width: 150px; border-radius: 20px; text-transform: capitalize;color:#FFFFFF">Create</v-btn>                          
                    </v-col>
                ...
                <v-row >
                    <v-col class="table mt-5">
                        ...
                        <div  class="d-flex  justify-end">
                            ...
                            <v-data-table
                                ...
                                :items="pinia.state.paymentMethod || []"
                                :loading="isloading"
                                :search="search"
                                style="background: transparent !important; color: #fff; width: 100%;"
                            >
                                <template v-slot:item.edit="{ item }">
                                <v-btn @click="navigateTo(`/dashboard/paymentMethod/${item?.id}`)" class="rounded-xl text-capitalize"  color="success">Edit</v-btn>
                                </template>
                    ...
                    <v-card  style="background: #12181F !important; color: #FFFFFF; border-radius: 15px;padding-bottom: 10px;">
                        ...
                        <v-card-text>Are you sure you want to delete {{ filteredItem?.name}}?</v-card-text>
                        <v-card-actions>
                        <v-spacer></v-spacer>
                        ...
                        <v-btn color="" :loading="isDeleting"  @click="delete_paymentMethod" style="background: red; font-size: medium; text-transform: capitalize; border-radius: 10px;">Confirm</v-btn>
                        </v-card-actions>
                    </v-card>
                    </v-dialog>
                    </v-col>
...
<script setup>
  ...
  import { deletePaymentMethod } from "@/composables/requests/payment"
  ...
  const handleDeleteModal = (id)=>{
      dialog.value = true
      filteredItem.value = pinia.state.paymentMethod.find(e => e.id ===  id )
  }



  
  const delete_paymentMethod = async() => {
    isDeleting.value = true
    try{
        const data = await deletePaymentMethod(filteredItem.value.id)
        if(data.success){
            push.success('Deleted successfully')  
            const filtered = pinia.state.paymentMethod.filter(e => e.id !=  filteredItem.value.id )
            pinia.setPaymentMethod(filtered)
        }else{
  ...
  const fetch_paymentMethod = async()=>{
      if(pinia.state.paymentMethod.length){
          return
        }else{
            isloading.value =  true
            await  get_paymentMethod()
  ...
  onMounted(() => {
      fetch_paymentMethod()
      console.log(pinia.state.paymentMethod)
  })
