            ...
            <v-row>
                ...
                <v-col sm="12" md="10" style="margin-top: 16px;">
                    ...
                    <v-col cols="12" md="6" style="margin-top: 24px;">
                        ...
                        <v-btn  class="icon"  @click.prevent="update_FAQs()"   :loading="isloading" append-icon="mdi:arrow-right" variant="tonal" height="60px" width="100%" rounded
...
<script setup>
    import { updateFAQs } from "@/composables/requests/FAQs"
    ...
    const update_FAQs = async()=>{
        ...
        try{
            const data = await updateFAQs(payload,currentId)
            if(data.success){
                push.success('Edited successfully')  

                const newData = pinia.state.FAQs.map(item=>{
                    if(item.id === currentId){
                        return {...item,...payload}
                    }
                    return item
                })
                pinia.setFAQs(newData)
                // get_banner()
